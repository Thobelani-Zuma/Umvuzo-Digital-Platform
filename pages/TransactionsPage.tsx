...
import { PlusIcon, TrashIcon, PrintIcon, CameraIcon } from '../components/icons/Icons';
...
export function TransactionsPage({ repName, addMultipleTransactions }: TransactionsPageProps) {
  ...
  const handleAddItem = () => {
    ...
    if (isNaN(weightValue) || weightValue <= 0) {
      setStatus({ message: 'Please enter a valid positive weight in kilograms.', type: 'error' });
      return;
    }

    const materialInfo = rateSheet.find(m => m.type === material);
    if (!materialInfo) {
      setStatus({ message: 'Please select a valid material from the rate sheet.', type: 'error' });
      return;
    }
  ...
  const scanImageForWeight = async (base64Image: string) => {
    ...
    try {
        ...
        if (!apiKey) {
            setScanStatus('error');
            setScanError("Gemini API key is not configured. Please enter weight manually or add GEMINI_API_KEY.");
            return;
        }
        ...
        if (!isNaN(weightValue) && weightValue > 0) {
            ...
            setScanStatus('error');
            setScanError("Could not detect a valid weight from the image. Please enter weight manually.");
        }
    } catch (error) {
        console.error("Error scanning image for weight:", error);
        setScanStatus('error');
        setScanError("Failed to scan image. Please enter weight manually.");
    }
  };
  ...
  const capturePhoto = () => {
    if (videoRef.current && canvasRef.current) {
        ...
        if (video.videoWidth === 0 || video.videoHeight === 0) {
            setScanStatus('error');
            setScanError("Camera feed not ready yet. Please wait a moment.");
            return;
        }
  ...
  const handlePrintReceipt = () => {
    ...
    if (!clientName.trim() || items.length === 0) {
        setStatus({ message: "Please enter a client name and add at least one item before printing.", type: 'error' });
        return;
    }
    ...
    // Try printing via hidden iframe (robust in sandboxed environments and prevents popup blocker)
    ...
    try {
      ...
      // Fallback if cross-document write is blocked
    }
    ...
    if (printWindow) {
        ...
        setStatus({ message: "Printing blocked by browser. Please allow popups or use browser print.", type: 'error' });
    }
  };
      ...
      {isCameraOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
            <div className="bg-white p-4 rounded-lg shadow-xl relative max-w-lg w-full">
                <h3 className="text-lg font-bold mb-4 text-center">Scan Weight from Image</h3>
