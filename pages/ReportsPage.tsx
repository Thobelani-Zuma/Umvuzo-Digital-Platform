...
export function ReportsPage({ transactions }: ReportsPageProps) {
  ...
  const handleGenerateReport = (type: ReportType) => {
    ...
    if (filteredTransactions.length === 0) {
      setNotice({ message: 'No transactions found for the selected filter to generate a report.', type: 'info' });
      return;
    }
  ...
  const handleShareReport = async (type: ReportType) => {
    setNotice(null);
    if (filteredTransactions.length === 0) {
      setNotice({ message: 'No transactions found for the selected filter to share.', type: 'info' });
      return;
    }
    ...
    try {
      ...
      if (!pdfBlob || pdfBlob.size === 0) {
        setNotice({ message: 'Unable to generate report data.', type: 'error' });
        return;
      }
      ...
      if (navigator.share && navigator.canShare && navigator.canShare({ files: [reportFile] })) {
          ...
          setNotice({ message: "Direct sharing is not supported by your browser. Please use the 'Download' button.", type: 'info' });
      }
    } catch (err) {
      if ((err as DOMException).name !== 'AbortError') {
          console.error("Share failed:", err);
          setNotice({ message: "Sharing was not completed. You can download the report instead.", type: 'error' });
      }
    } finally {
      setIsSharing(null);
    }
